...
def load_dependencies():
    ...
    vqa_task = VQATask(encoder = encoder, retriever = retriever, object_filter = obj_filter, vlm_pipeline = vlm)
            ...
            if not results:
                ...
                for idx, res in enumerate(results):
                    ...
                    with col:
                        ...
                        f_id = res["frame_id"]
                        score = res["score"]
                        k_path = res["keyframe_path"]

                        try:
                            if os.path.exists(k_path):
                                st.image(k_path, use_container_width = True)
                            else:
                                st.info(f"Image not available: {Path(k_path).name}")
    ...
    if st.button("Ask", type="primary"):
        if not question.strip():
            ...
            with st.spinner("Analyzing..."):
                results = vqa_task.execute(question, top_k=top_k)
