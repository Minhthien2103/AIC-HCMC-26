...
# =============================================================================
DEFAULT_QUERIES_DIR    = "data/test"   # input folder
DEFAULT_OUTPUT_DIR     = "submission"     # output folder
DEFAULT_TOP_K          = 600           # candidates per KIS / TRAKE query
DEFAULT_VQA_TOP_K      = 350           # candidates per VQA query (VLM is slow)
DEFAULT_TRAKE_VIDEOS   = 200            # videos re-ranked in TRAKE phase B
DEFAULT_PARAPHRASE_N   = 5             # number of expanded query variants (RRF)
DEFAULT_MAX_ROWS       = 1000          # max CSV rows written per query
...
def build_pipeline(args: argparse.Namespace):
    ...
    if ENABLE_VLM and not args.no_vlm:
        LOGGER.info("VLM will be lazy-loaded on first VQA/TRAKE query.")
    ...
    kis   = KIStask(encoder, retriever, object_filter=object_filter, vlm_pipeline=vlm)
    ...
    if spec.query_type == "qa":
        ...
        for r in results:
            try:
                ...
                if any(bad in ans_lower for bad in ["not visible", "không rõ", "khong ro", "không thấy", "không có", "không thể nhận biết", "khong the nhan biet", "khng th? nh?n bi?t"]):
                    continue
